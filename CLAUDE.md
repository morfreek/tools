...
- Seguimiento (`src/pages/ProjectDetail.jsx`, ruta `/projects/:id/detail`): pendientes de la última revisión, último cambio registrado y gráfico de evolución (`ReviewTrendChart`, SVG propio). La lógica vive en `@u/reviewTracking` (funciones puras con pruebas en `tests/reviewTracking.test.js`). Las revisiones nuevas copian el estado de la anterior, por eso el "último cambio" busca el par consecutivo más reciente con diferencias. El gráfico usa los tokens `--grafico-bien/regular/deficiente` de `index.css`, validados para daltonismo con la skill dataviz: no reemplazarlos por `--ok/--ambar/--rojo`, que como trío apilado no pasan.
